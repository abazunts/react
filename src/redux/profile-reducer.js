    ...
    switch (action.type) {
        ...
        case SET_EDIT_CONTACTS:
            return {
                ...
                profile: {
                    ...state.profile,
                    contacts: {...state.profile.contacts, [action.fieldName]: action.fieldValue}
                }
            };
...
export const putProfile = () => async (dispatch, getState) => {
    ...
    let response = await apiService.putProfile(profilePage.profile)
...
export const putPhotos = (formData) => async (dispatch) => {
